...
if __name__ == "__main__":
    ...
    individual_output = False
    #LehmerPy V1.6 Stable
    version = 16
    beta = False
...
def Lucas_lehmer_prog_main_range(p_start_int, start_var, var, max, Mersenne_primes_queue, finished, ex, odd, arguments, maxspeed, progress):
    try:
        if "-dc" in arguments or (version_get and release_ver < 10):
            colors.rmcolor()
        else:
        ...
        if odd or maxspeed:
            ...
            for p in range(p_start_int + start_var*2-var_sub, max + 1, var*2):
...
/_____/\___/_/ /_/_/ /_/ /_/\___/_/  /_/    \__, /  
                                           /____/   """]
    acsii_title_op = int(time.time()*100) % len(acsii_title) if version_get and release_ver <= 10 else (int(time.time()*100) % (len(acsii_title) - 1)) + 1
    ...
    if str(mode) == "1" or mode == "range" or mode == "Range" or mode == "R" or mode == "r":
        ...
        if not noprocess:
            ...
            for num in range(core_count):
                ...
                multi = Process(target=Lucas_lehmer_prog_main_range, name=f"Worker {num + 1}", args=(p_start_int, num, core_count, max_p_value, Mersenne_primes_queue, finished, ex, odd, arguments, maxspeed, progress))
                multi.start()
                    ...
                    with open(file_path+f"M{prime}.txt", "r", encoding='utf-8') as indread:
                        if os.path.exists(file_path+f"M{prime}.txt"):
                            ...
                            if str(2**prime-1) != str(contents):
                                print(f"Changing contents of {file_path + ioname}")
                                while perm_stat:
                                    try:
                                ...
                                print(f"{ioname} already exists")                            
                except FileNotFoundError:
                    while perm_stat:
                        try:
                            print(f"Generating {ioname}")
                            with open(file_path+f"M{prime}.txt", "w") as indout:
                                indout.write(str(2**prime-1))
                            perm_stat = False
                        except PermissionError:
    ...
    if str(mode) == "2" or mode == "confirm" or mode == "Confirm" or mode == "C" or mode == "c":
        ...
        if not noprocess:
            ...
            for num in range(core_count):
                ...
                multi = Process(target=Lucas_lehmer_confirm, name=f"Worker {num + 1}", args=(p, passes, num, core_count, Mersenne_confirm_status, finished, ex, residue, arguments))
                multi.start()
            if not "-1" in str(loadani_speed):
